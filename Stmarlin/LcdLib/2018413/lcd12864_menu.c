...
//    {
//      cardInsertFlag = 1;
//      card_readFileListInfo(); //读取SD卡文件数目和文件名列表
//      
//      sprintf((char *)CardMenu.title," Card File List ");//CardMenu.title = " Card File List ";
...
//    {
//      cardInsertFlag = 0;
//      myfree(CardMenu.subMenus);
//      myfree(CardMenu.func);
////      for(uint8_t i = 0; i < totgconum; ++i)
////      {
////        CardMenu.menuItems[i] = "            ";
////      }
////      sprintf((char *)CardMenu.title,"  NO SD Card!   ");//CardMenu.title = "  NO SD Card!   ";
////      CardMenu.displayUpdate_f = &lcd_displayUpdate_cardMenu;//&lcd_displayUpdate_printingFinishedMenu;
...
//	fnn = (u8 *)(CardMenu.menuItems[CardMenu.selected]); //读取当前打印文件的文件名
	...
	//StatusMenu.menuItems[0] = (uint8_t *)printingFilename; //将文件名存储到状态菜单条目中，用于显示当前打印文件名	
	  //2018/4/12 (待测试) (测试成功)
  fnn = CardMenu.dynamicMenuItems[CardMenu.selected];
  StatusMenu.menuItems[0] = fnn;
...
//						lcdDisplayUpdate = 1; //屏幕显示更新标记置位  //不去掉会以当前形式更新下一级菜单，致使显示bug
    ...
    LCD12864_ShowString(0,0,"                ");
		...
		for(i=1;i<4;i++)
		{
			if(strlen((const char *)CurrentMenu->dynamicMenuItems[i-1+CurrentMenu->range_from])>14) //超出显示范围显示部分内容
			{
				strncpy((char *)temp_string,(const char *)CurrentMenu->dynamicMenuItems[i-1+CurrentMenu->range_from],14);
        LCD12864_ShowString(i,0,"                ");
			...
			{
			  LCD12864_ShowString(i,0,"                ");
        LCD12864_ShowString(i,1,CurrentMenu->dynamicMenuItems[i-1+CurrentMenu->range_from]);
			}
		}
		lineSeclected = CurrentMenu->selected; //读取当前菜单的选中的条目号
		lineSeclected = 3-(CurrentMenu->range_to - lineSeclected);	//转化位适合屏幕显示的当前选中行
    ...
    //选中行文件名显示不下滚动显示
		if(strlen((const char *)CurrentMenu->dynamicMenuItems[CurrentMenu->selected])>14)
		{
			strcpy(seclected_filename,(const char *)CurrentMenu->dynamicMenuItems[CurrentMenu->selected]);
			for(m=0;m<14;m++)
			{
...
//	u16 curindex;		     //Gcode文件当前索引
    ...
    CardMenu.dynamicMenuItems = mymalloc(sizeof(char *) * CardMenu.itemCount);  //为存放SD卡文件名列表申请空间
...
//					strncpy((char *)temp_fn[curindex],(const char*)fn,14); //将文件名前14个字符存储到1个二维数组中
          ...
          CardMenu.dynamicMenuItems[curindex] = temp_fn[curindex]; //将二维数组中的字符串存取到CardMenu.dynamicMenuItems数组中，即初始化文件列表名称
					curindex++;
				}	    
			} 
		}   
