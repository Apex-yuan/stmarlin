...
// Variables used by The Stepper Driver Interrupt 步进电机中断中用到的变量
static unsigned char out_bits;        // The next stepping-bits to be output 步进方向位
static long counter_x,       // Counter variables for the bresenham line tracer //bresenham线生成算法的计数变量
            counter_y, 
            counter_z,       
            counter_e;
...
static unsigned short acc_step_rate; // needed for deccelaration start point 为了计算减速的起点
...
/*static inline*/ unsigned short calc_timer(unsigned short step_rate) {  //计算中断时间（中断时间对应的频率就是步进电机的速度）
  ...
  //原固件中此处的时间计算采用查表法（可能是受限于arduino的计算能力或许还有其它原因，尚不清楚），此处采用直接计算的方式
  //此处的计算方式来自于Marlin_STM32固件。
  timer = 2000000/step_rate - 1;
	 
  if(timer < 100) { timer = 100; printf(MSG_STEPPER_TO_HIGH); printf("%d",step_rate); printf("\n"); }//(20kHz this should never happen)
  return timer;
}

// Initializes the trapezoid generator from the current block. Called whenever a new 
// block begins. //初始化当前block的梯形发生器，每当一个新的block开始时都要调用（在步进电机中断中执行，一个block中，该函数只执行一次）
/*static inline*/ void trapezoid_generator_reset(void) {
  ...
  deceleration_time = 0; //减速段的时间清零
  // step_rate to timer interval  //将步进速率转化为定时器时间间隔
  ...
  acc_step_rate = current_block->initial_rate; //设定加速度段的起始速度
  acceleration_time = calc_timer(acc_step_rate); //计算加速度段的起始速度对应的中断时间
...
// It pops blocks from the block_buffer and executes them by pulsing the stepper pins appropriately. //从block缓冲区中取出block并通过适当的脉冲步进脚执行它们
//block中的step_event_count（步事件计数：各轴中要移动步数的最大值）利用	step_event_count作为假想的长轴，以其为基准利用bresenham算法计算XYZE各轴要运动的步数 
//定时器4中断服务程序
void TIM4_IRQHandler(void) {  //TIM4中断
  if(TIM_GetITStatus(TIM4, TIM_IT_Update) != RESET) {	 //检查指定的TIM中断发生与否:TIM 中断源   	   
    ...
    // If there is no current block, attempt to pop one from the buffer  //如果没有当前block，尝试从缓冲区中取出	 
	  if (current_block == NULL) {  // Anything in the buffer?  //该部分只会在每个block开始时执行一次	     
      ...
      if(current_block != NULL) {      
        ...
        counter_x = -(current_block->step_event_count >> 1);  //step_event_count所有轴要移动步数的最大值 
        counter_y = counter_x;
        counter_z = counter_x;
        counter_e = counter_x;
    ...
    if (current_block != NULL) { 
      ...
      // Set the direction bits (X_AXIS=A_AXIS and Y_AXIS=B_AXIS for COREXY) //设定方向位
      ...
      /* X轴 */
        ...
        if(check_endstops) {  //CHECK_ENDSTOPS开启限位开关检测      
            ...
            old_x_min_endstop = x_min_endstop; //记录x轴限位开关状态的历史值
      ...
      //step_loops会依据clac_timer()函数来确定	
      for(uint8_t i=0; i < step_loops; i++) {  // Take multiple steps per interrupt (For high speed moves)	//每个中断周期走多步（对于高速运动）
        //MYSERIAL_checkRx(); // Check for serial chars. //该函数不能加在这，若加在此处打印机将不能正常运行（喷头在打印过程中总是莫名其妙的沿X轴或Y轴运动到最大位置或最小位置再返回）
        ...
        if (counter_x > 0) {
          WRITE(X_STEP_PIN, !INVERT_X_STEP_PIN); //true
          counter_x -= current_block->step_event_count;  
          count_position[X_AXIS]+=count_direction[X_AXIS];  //计算X轴的当前位置 
          WRITE(X_STEP_PIN, INVERT_X_STEP_PIN); //false
        }  
        counter_y += current_block->steps_y;
        if (counter_y > 0) {
          WRITE(Y_STEP_PIN, !INVERT_Y_STEP_PIN); //true
          counter_y -= current_block->step_event_count; 
          count_position[Y_AXIS]+=count_direction[Y_AXIS]; //计算Y轴的当前位置 			
         WRITE(Y_STEP_PIN, INVERT_Y_STEP_PIN); //false
        }    
        counter_z += current_block->steps_z;
        if (counter_z > 0) 
        {
          WRITE(Z_STEP_PIN, !INVERT_Z_STEP_PIN); //true      
          counter_z -= current_block->step_event_count;
          count_position[Z_AXIS]+=count_direction[Z_AXIS]; //计算Z轴的当前位置 
          WRITE(Z_STEP_PIN, INVERT_Z_STEP_PIN); //false
        }
        #ifndef ADVANCE  	  
          counter_e += current_block->steps_e;
          if (counter_e > 0) 
        ...
        step_events_completed += 1;   //更新block的完成度
        if(step_events_completed >= current_block->step_event_count) break;  //当前block执行完毕
      }
    ...
    /* 梯形匀速阶段 */  
       ...
       // ensure we're running at the correct step rate, even if we just came off an acceleration  //确保运行的速率正确，即使刚刚在加速
...
void st_init(void) {	
   ...
   //初始化步进电机引脚并将步进电机设为关闭状态
	...
	//初始化X轴步进电机驱动的引脚
	 ...
	 WRITE(X_STEP_PIN, INVERT_X_STEP_PIN); //脉冲脚初始化为false
   disable_x();	//关闭X轴步进电机使能
  
	 //初始化Y轴步进电机驱动的引脚
	 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5|GPIO_Pin_6|GPIO_Pin_7;		
	 GPIO_Init(GPIOF, &GPIO_InitStructure);	
	 WRITE(Y_STEP_PIN, INVERT_Y_STEP_PIN); //脉冲脚初始化为false
	 disable_y();	//关闭Y轴步进电机使能

   //初始化Z轴步进电机驱动的引脚
	 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4;			
	 GPIO_Init(GPIOF, &GPIO_InitStructure);
	 WRITE(Z_STEP_PIN, INVERT_Z_STEP_PIN); //脉冲脚初始化为false
   ...
   //初始化E0轴步进电机驱动的引脚
	 ...
	 GPIO_Init(GPIOE, &GPIO_InitStructure);
	  WRITE(E0_STEP_PIN, INVERT_E_STEP_PIN); //脉冲脚初始化为false
   disable_e0();	//关闭e0轴步进电机使能	 
   
	 //初始化E1轴步进电机驱动的引脚
	 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3|GPIO_Pin_4|GPIO_Pin_5;			
	 GPIO_Init(GPIOE, &GPIO_InitStructure);
	 WRITE(E1_STEP_PIN, INVERT_E_STEP_PIN); //脉冲脚初始化为false
  ...
  enable_endstops(true); //激活限位开关检测 // Start with endstops active. After homing they can be disabled
  //sei();开启全局中断，后面找一下在STM32中开启全局中断的函数来替代，似乎没有程序也可正常运行
}

// Block until all buffered steps are executed  阻塞直到所有的缓冲区步数都被执行
...
//	 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10;	
//	 GPIO_Init(GPIOF, &GPIO_InitStructure);
//	  WRITE(X_STEP_PIN,INVERT_X_STEP_PIN);//X_STEP_PINa=INVERT_X_STEP_PIN; //脉冲脚初始化为false
//   disable_x();	//关闭X轴步进电机使能
//  
//	 //初始化Y轴步进电机驱动的引脚
//	 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5|GPIO_Pin_6|GPIO_Pin_7;		
//	 GPIO_Init(GPIOF, &GPIO_InitStructure);	
//	  WRITE(Y_STEP_PIN,INVERT_Y_STEP_PIN);//Y_STEP_PINa=INVERT_Y_STEP_PIN; //脉冲脚初始化为false
//	 disable_y();	//关闭Y轴步进电机使能

//   //初始化Z轴步进电机驱动的引脚
//	 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4;			
//	 GPIO_Init(GPIOF, &GPIO_InitStructure);
//	  WRITE(Z_STEP_PIN,INVERT_Z_STEP_PIN);//Z_STEP_PINa=INVERT_Z_STEP_PIN; //脉冲脚初始化为false
//   disable_z();	//关闭Z轴步进电机使能	 
...
//	 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;			
//	 GPIO_Init(GPIOE, &GPIO_InitStructure);
//	   WRITE(E0_STEP_PIN,INVERT_E_STEP_PIN);//E0_STEP_PINa=INVERT_E_STEP_PIN; //脉冲脚初始化为false
//   disable_e0();	//关闭e0轴步进电机使能	 
//   
//	 //初始化E1轴步进电机驱动的引脚
//	 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3|GPIO_Pin_4|GPIO_Pin_5;			
//	 GPIO_Init(GPIOE, &GPIO_InitStructure);
//	  WRITE(E1_STEP_PIN,INVERT_E_STEP_PIN);//E1_STEP_PINa=INVERT_E_STEP_PIN; //脉冲脚初始化为false
//   disable_e1();	//关闭e1轴步进电机使能	 
