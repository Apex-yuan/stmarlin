...
///头文件的包含顺序：按稳定性递增的顺序排列，即越稳定（越底层，不常修改）的头文件越往下面放置
/* STMARLIN */
#include "planner.h"
#include "stepper.h"
...
void setup(void) {	
	printf("start\n");
  SERIAL_ECHO_START;
  printf(MSG_STMARLIN); //打印Stmarlin
  printf(VERSION_STRING); //打印版本号
  printf("\n");
...
{ 
  ...
  {	
    serial_char = MYSERIAL_read(); //从串口缓冲区读取命令字符
    ...
    if(serial_char == '\n' ||
      ...
      if(!comment_mode) {  //comment_mode 注释，如果不是注释   
        ...
        if(strchr(cmdbuffer[bufindw], 'N') != NULL) {  //如果该字符串中有字符‘N’N为行码 //extern char *strchr(const char *s,char c);功能：查找字符串s中首次出现字符c的位置 说明：返回首次出现c的位置的指针，如果s中不存在c则返回NULL                                                             
          ...
          if(strchr(cmdbuffer[bufindw], '*') != NULL) {  //当前字符串中有“*”// *号为标记码  
            ...
            if((int)(strtod(&cmdbuffer[bufindw][strchr_pointer - cmdbuffer[bufindw] + 1], NULL)) != checksum) {  //*号后面的数值不等于标记码的数值 //函数 strtod() 用来将字符串转换成双精度浮点数(double)				                                                               
              SERIAL_ERROR_START;  //串口打印“ERROR”
        ...
        if((strchr(cmdbuffer[bufindw], 'G') != NULL)) //当前字符串中有字符“G”
          ...
          switch((int)((strtod(&cmdbuffer[bufindw][strchr_pointer - cmdbuffer[bufindw] + 1], NULL)))) {  //读取字符G后面的数字						
							...
							case 2:
							case 3:
								if(Stopped == false) { // If printer is stopped by an error the G[0-3] codes are ignored. //如果打印机因错误处于停止状态，G[0-3]代码会被忽略掉。
                  #ifdef SDSUPPORT
                    if(card.saving)
                      break;
							    #endif //SDSUPPORT
								  ...
								  //LCD_MESSAGEPGM(MSG_STOPPED);  //lcd屏显示停止信息
                }
								break;
							default:
								break;
      ...
      serial_count = 0; //reset buffer  //复位缓冲区
    }
    else {
      if(serial_char == ';') comment_mode = true; //串口读取到分号,分号后面为注释信息
      if(!comment_mode) cmdbuffer[bufindw][serial_count++] = serial_char; //如果不是注释内容，将串口信息存储到命令缓冲区
    }
  }
#ifdef SDSUPPORT   
...
static void axis_is_at_home(int axis)  //设定轴的当前位置为原点位置（之后回原点的位置）
{
...
static void homeaxis(int axis)  //轴回坐标原点
{
  ...
  if(axis==X_AXIS ? HOMEAXIS_DO(X) :
     ...
     0) {
    ///当前轴以1.5倍该轴最大长度的距离向原点移动，触碰到限位开关会停止   
    ...
    //当前轴向原点反方向移动home_retract_mm[axis]的距离   
    current_position[axis] = 0;
    plan_set_position(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS]);
    ...
    axis_is_at_home(axis);  //设定轴的当前位置为原点位置
    destination[axis] = current_position[axis]; //目标位置清零
    feedrate = 0.0;  //进给速率清零
    endstops_hit_on_purpose(); //复位限位开关触发，否则轴回到坐标原点后便无法后续运动
    axis_known_position[axis] = true;
  }
}
#define HOMEAXIS(LETTER) homeaxis(LETTER##_AXIS)  //以宏的形式调用轴回坐标原点的函数
...
void get_coordinates() {  //获取G指令中的坐标值 
...
void get_arc_coordinates(void) {  //获取G指令中的圆弧坐标
  #ifdef SF_ARC_FIX
...
void prepare_move(void) {   //准备移动
  ...
  previous_millis_cmd = millis();  //记录机器当前运行的时间长度
...
#if defined(CONTROLLERFAN_PIN)  //(非活动)
...
bool setTargetedHotend(int code) {  //设定热端的目标温度
  tmp_extruder = active_extruder;
  if(code_seen('T')) {  //读取到了字符T 
    tmp_extruder = code_value(); //读取T后面的数字，即为要设定温度的喷头编号
    if(tmp_extruder >= EXTRUDERS) {
      SERIAL_ECHO_START;
      switch(code) {
        case 104:
...
***********************
*/
///*static inline*/ void gcode_G0_G1(void) {  
//  if(Stopped == false) 
//  {
//    get_coordinates(); // For X Y Z E F
//    prepare_move();
...
//  if(code_seen('P')) dwell_ms = code_value(); // milliseconds to wait  //毫秒等待
//  if(code_seen('S')) dwell_ms = code_value() * 1000; // seconds to wait  //秒等待
//  st_synchronize();  //synchronize使……合拍，同步
//  dwell_ms += millis();  // keep track of when we started waiting  //当开始等待时保持轨迹
//  previous_millis_cmd = millis(); //记录历史值
...
//  endstops_hit_on_purpose();
//}
///*static inline*/ void gcode_G92(void) {
//  if(!code_seen(axis_codes[E_AXIS]))
//    st_synchronize();
//    for(char i=0; i < NUM_AXIS; i++) {
//      if(code_seen(axis_codes[i])) {
...
//  //int8_t i;
//  if(code_seen('G')) {  //读取到了字符G
//    switch((int)code_value()) {  //读取G后面的数值
//      case 0: // G0 -> G1
//      case 1: // G1
//        gcode_G0_G1();
//        break;
...
//      case 10: // G10 retract  //回抽
//        break;
//      case 11: // G10 retract_recover  //回抽恢复
//        break;
//      #endif //FWRETRACT
//      case 28: //G28 Home all Axis one at a time  //所有轴立刻归位
//        gcode_G28();
...
//    }
//  }
//  else if(code_seen('M')) {	 //读取到了字符M
//    switch((int)code_value())  //读取M后面的数值
//    {  
//	    case 0: // M0 - Unconditional stop - Wait for user button press on LCD
//	    case 1: // M1 - Conditional stop - Wait for user button press on LCD
//			  break;
