...
unsigned long axis_steps_per_sqr_second[NUM_AXIS]; //对应的以步为单位的加速度 steps/s^2
...
// NOTE: Removed modulo (%) operator, which uses an expensive divide and multiplication. 删除%运算符因为它使用昂贵的除法和乘法
...
void planner_reverse_pass() {
  ...
  //Make a local copy of block_buffer_tail, because the interrupt can alter it //做一个本地的复制，因为中断可能会改变它。
...
void check_axes_activity(void) {  //检测轴的活跃性
...
/* 在缓冲区中添加一个新的线性移动。stepsx、y和z是mm中的绝对位置，微秒指定移动应该执行多少微秒。
   为了加速计算，调用者也必须以毫米的长度提供线路的物理长度。 */
void plan_buffer_line(const float x, const float y, const float z, const float e, float feed_rate, const uint8_t extruder) {
  // Calculate the buffer head after we push this byte  //在存入这个字节后计算缓冲区的头指针
  ...
  // Number of steps for each axis //每个轴要移动的step数
  ...
  // Compute direction bits for this block 为这个block计算方向位（通过移位运算将各轴的方向信息存储到一个变量中）
  ...
  // slow down when de buffer starts to empty, rather than wait at the corner for a buffer refill 当缓冲区开始清空时减速，而不是在角落等待缓冲区重新填充
  #ifdef OLD_SLOWDOWN  //（非活动）
    if(moves_queued < (BLOCK_BUFFER_SIZE * 0.5) && moves_queued > 1)
      feed_rate = feed_rate*moves_queued / (BLOCK_BUFFER_SIZE * 0.5); 
  #endif

  #ifdef SLOWDOWN
    //segment time im micro seconds //细分时间
    segment_time = round(1000000.0/inverse_second);
    if ((moves_queued > 1) && (moves_queued < (BLOCK_BUFFER_SIZE * 0.5))) {
      if (segment_time < minsegmenttime) { 
        // buffer is draining, add extra time.  The amount of time added increases if the buffer is still emptied more.
        // 缓冲区正在消耗，增加额外的时间。如果缓冲区仍然被清空，则时间的增加量会增加。
...
// availible for new blocks. 当当前block不在需要时调用，删除这个block并使新的block可以得到这块内存
