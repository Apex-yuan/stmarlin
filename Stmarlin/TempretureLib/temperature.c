...
static void updateTemperaturesFromRawValues(void) {	
	for(uint8_t e=0;e<EXTRUDERS;e++) {  //读取加热头的目前温度
...
void tp_init() {		
  ...
  for(uint8_t e = 0; e < EXTRUDERS; e++) {   // populate with the first value 
...
size_t
  ...
  switch(temp_state) {
    case 0: // Measure TEMP_0
      #if defined(TEMP_0_PIN) 
		   raw_temp_0_value += GET_ADC(TEMP_0_PIN);
      #endif
      temp_state = 1;
      break;
    case 1: // Measure TEMP_1
      #if defined(TEMP_1_PIN) 
		   raw_temp_1_value += GET_ADC(TEMP_1_PIN);
      #endif
      temp_state = 2;
      break;
    case 2: // Measure TEMP_2
      #if defined(TEMP_2_PIN) 
		   raw_temp_2_value +=GET_ADC(TEMP_2_PIN);
      #endif
      temp_state = 3;
      break;
    case 3: // Measure TEMP_BED
      #if defined(TEMP_BED_PIN) 
		   raw_temp_bed_value += GET_ADC(TEMP_BED_PIN); //28ms
      #endif
      temp_state = 0;
      temp_count++;  //4ms执行一次
      break;  
...
inline float degTargetBed(void) {   
  return target_temperature_bed;
}
//设置加热头的目标温度
//在目标温度大于0时直接打开挤出机0的风扇，后面要进行优化 ，使热端加热到目标温度后再打开风扇，提高效率！
inline void setTargetHotend(const float celsius, uint8_t extruder) { //celsius：摄氏度
...
inline bool isHeatingHotend(u8 extruder) { //判断挤出头是在加热还是在冷却
  return target_temperature[extruder] > current_temperature[extruder];
}

inline bool isHeatingBed(void) {
  return target_temperature_bed > current_temperature_bed;
}

inline bool isCoolingHotend(u8 extruder) {  
  return target_temperature[extruder] < current_temperature[extruder];
}

inline bool isCoolingBed(void) {
  return target_temperature_bed < current_temperature_bed;
}
inline void autotempShutdown(void) {
	#ifdef AUTOTEMP
	if(autotemp_enabled) {
